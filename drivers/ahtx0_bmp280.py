...
class AHT20:   # AHT10驱动协议但AHT20类名字保持不变
    AHTX0_I2CADDR_DEFAULT = 0x38
    ...
    def __init__(self, i2c, address=AHTX0_I2CADDR_DEFAULT):
        self.i2c = i2c
        self.address = address
        time.sleep_ms(50)  # 上电延时，保证芯片准备好

    def read(self):
        # 触发一次测量
        self.i2c.writeto(self.address, bytes([self.AHTX0_CMD_TRIGGER, 0x33, 0x00]))
        time.sleep_ms(80)

        data = self.i2c.readfrom(self.address, 7)
        if (data[0] & self.AHTX0_STATUS_BUSY):
